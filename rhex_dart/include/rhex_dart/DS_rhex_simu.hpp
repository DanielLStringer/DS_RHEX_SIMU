...
namespace rhex_dart {
    ...
    public:
		...
		std::vector <double> phases_1
		std::vector <double> phases_2	
		double Ep_1;
		double Ep_2;
		double t_switch=2;
        ...
        DSRhexDARTSimu(const std::vector<double>& ctrl, robot_t robot, int world_option = 0, double friction = 1.0, std::vector<rhex_dart::RhexDamage> damages = {}) : _covered_distance(0.0),
                                                                          _energy(0.0),
...
//My stuff in between these
///////////////////////////////
			for(unsigned i = 0; i<6; i++){
				phases_1[i]=ctrl[i]*PI;
			}
						
						
			for(unsigned j = 6; i<12; j++){
				phases_2[j]=ctrl[j]*PI;
			}
...
///////////////////////////////
        ...
        void run(double duration = 5.0, bool continuous = false, bool chain = false)
        {
...
#endif
            {
				...
				if((_world->getTime() - old_t)<t_switch){
					phases=phases_1;
					Ep=Ep_1;
				}else if(((_world->getTime() - old_t)>=t_switch){
					phases=phases_2;
					Ep=Ep_2;
    ...
    protected:
       ...
       // rhex_dart::OscControl _controller;
