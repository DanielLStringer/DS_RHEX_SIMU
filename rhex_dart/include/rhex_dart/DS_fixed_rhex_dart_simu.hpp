...
namespace rhex_dart {
    ...
    public:
        ...
        void run(double duration = 15.0, bool continuous = false, bool chain = false)
        {
			...
			double Ep=0.5;
			phases[0]=-1*Pi;			
			//std::cout<<"phases[0], meant to be -1*Pi:    "<< phases[0]<<std::endl;
			phases[1]=0;
			phases[2]=1*Pi;
			phases[3]=0;
			phases[4]=-1*Pi;
			...
			//Quadruped gait
			/*double Ep=2.0/3.0;
			//std::cout<<"Ep as 2.0/3.0:   "<< Ep<<std::endl;
			//double Ep=0.666666;
			//std::cout<<"Ep as decimal:   "<< Ep<<std::endl;						
			phases[0]=(-2.0/3.0)*Pi;
			//std::cout<<"phases[0], meant to be (-2/3)*Pi:    "<< phases[0]<<std::endl;				
			phases[1]=(-2.0/3.0)*Pi;			
			//std::cout<<"phases[1], meant to be (-2/3)*Pi:    "<< phases[1]<<std::endl;				
			phases[2]=(4.0/3.0)*Pi;
			phases[3]=(-2.0/3.0)*Pi;
			phases[4]=(-2.0/3.0)*Pi;
			phases[5]=(4.0/3.0)*Pi;*/
