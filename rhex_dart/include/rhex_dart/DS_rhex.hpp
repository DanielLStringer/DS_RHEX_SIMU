...
Slightly edited by DS

*/


#ifndef DS_RHEX_DART_RHEX_HPP
#define DS_RHEX_DART_RHEX_HPP
...
namespace rhex_dart {
    ...
    struct DS_RhexDamage {
        DS_RhexDamage() {}
        DS_RhexDamage(const std::string& type, const std::string& data, void* extra = nullptr) : type(type), data(data), extra(extra) {}
    ...
    public:
        ...
        DS_Rhex(const std::string& model_file, const std::string& robot_name, const std::vector<std::pair<std::string, std::string>>& packages, bool is_urdf_string, std::vector<DS_RhexDamage> damages) : _robot_name(robot_name), _skeleton(_load_model(robot_name, model_file, packages, is_urdf_string))
        {
        ...
        DS_Rhex(const std::string& model_file, const std::string& robot_name, bool is_urdf_string, std::vector<DS_RhexDamage> damages) : DS_Rhex(model_file, robot_name, std::vector<std::pair<std::string, std::string>>(), is_urdf_string, damages) {}

        DS_Rhex(dart::dynamics::SkeletonPtr skeleton, std::vector<DS_RhexDamage> damages) : _skeleton(skeleton)
        {
            assert(_skeleton != nullptr);
            _set_damages(damages);
        ...
        std::vector<DS_RhexDamage> damages() const
        {
            return _damages;
        }
        ...
        void lock_hip_joints() {
			//std::cout << "Lock_hip_joints" << std::endl;
        ...
        void unlock_hip_joints() {
			std::cout << "unlock_hip_joints" << std::endl;
		...
		void velocity_hip_joints() {
			std::cout << "velocity_hip_joints" << std::endl;
            for(size_t i = 0; i < 6; i++){	
                auto jnt = _skeleton->getJoint("body_joint_" + std::string(1, i +'0'));
                jnt->setActuatorType(dart::dynamics::Joint::VELOCITY);
            }
        }
    ...
    protected:
        ...
        void _set_damages(const std::vector<DS_RhexDamage>& damages)
        {
            _broken_legs.clear();
            _damages = damages;
        ...
        std::vector<DS_RhexDamage> _damages;
