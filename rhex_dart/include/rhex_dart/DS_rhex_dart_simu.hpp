...
#ifndef DS_RHEX_DART_SIMU_HPP
#define DS_RHEX_DART_SIMU_HPP
...
namespace rhex_dart {
    ...
    template <typename DS_Simu, typename robot>
    struct Refresh {
        Refresh(DS_Simu& simu, std::shared_ptr<robot> rob, const Eigen::Vector6d& init_trans)
    ...
    public:
        ...
        DSRhexDARTSimu( robot_t robot, int world_option = 0, double friction = 1.0, std::vector<rhex_dart::DS_RhexDamage> damages = {}) : _covered_distance(0.0),
                                                                          _energy(0.0),
                                                                          ...
                                                                          _break(false)
        {
            ...
            _robot = robot;
			std::cout<<"DS_RHEX_DART_SIMU_HPP"<<std::endl;
        ...
        void run(double duration = 15.0, bool continuous = false, bool chain = false)
        {
...
#endif
            {
                ...
                if (_world->getTime() >= 2){
                    boost::fusion::for_each(_safety_measures, Refresh<DSRhexDARTSimu, DS_Rhex>(*this, rob, init_trans));
                // update visualizations
                boost::fusion::for_each(_visualizations, Refresh<DSRhexDARTSimu, DS_Rhex>(*this, rob, init_trans));
				}
                ...
                if (index % _desc_period == 0) {
                    // update descriptors
                    boost::fusion::for_each(_descriptors, Refresh<DSRhexDARTSimu, DS_Rhex>(*this, rob, init_trans));
                }
    ...
    protected:
		...
		void _add_later_hill(double friction = 1.0)
        {
            ...
            Eigen::Isometry3d tf(Eigen::Isometry3d::Identity());
           // tf.translation() = Eigen::Vector3d(3.2, 0.0, -4.0);
			tf.translation() = Eigen::Vector3d(4.0, 0.0, -4.4);
            hbody->getParentJoint()->setTransformFromParentBodyNode(tf);
        ...
        void _add_thin_pipe(double friction = 1.0)
        {
            ...
            for (size_t i = 0; i < 2; ++i)
            {
                ...
                // Put the body into position
                Eigen::Isometry3d tf(Eigen::Isometry3d::Identity());
                tf.translation() = Eigen::Vector3d(1.1, 0.0, 0.0);
